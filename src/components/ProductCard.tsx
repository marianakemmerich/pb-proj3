...
import { IProducts, ICartProduct } from '../types/Products'
...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const addToCart = () => {
    ...
    const existingProduct = cart.find((item: ICartProduct) => item.id === product.id)

    if (existingProduct) {
      existingProduct.quantity += 1
    } else {
      const productWithQuantity: ICartProduct = { ...product, quantity: 1 }
    ...
    >
      ...
      {isHovered && (
        <div className='font-poppins absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 flex flex-col justify-center items-center space-y-2'>
          <button 
            className='bg-white font-semibold text-golden w-4/5 px-4 py-2'
            onClick={addToCart}
          >
            Add to Cart
          </button>
          <div className='text-white text-base flex space-x-2'>
            <a className='flex items-center px-2 py-1'>
              <img src={share} className='w-[14px] mr-1'/>Share
            </a>
            <a className='flex items-center px-2 py-1'>
              <img src={compare} className='w-[14px] mr-1'/>Compare
            </a>
            <a className='flex items-center px-2 py-1'>
              <img src={like} className='w-[14px] mr-1'/>Like
            </a>
          </div>
        </div>
      )}
